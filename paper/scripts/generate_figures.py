...
def benchmarks():
    ...
    labels, values = ["load (s)", "total rows"], [0.5, 9352]
    if baseline.exists():
        jsons = sorted(baseline.glob("*.json"))
        if jsons:
            data = json.loads(jsons[-1].read_text())
            rows = data.get("per_table_rows", {})
            if rows:
                labels = [textwrap.shorten(k, 12) for k in list(rows.keys())[:5]]
                values = list(rows.values())[:5]
            if "load_seconds" in data:
                labels = ["load (s)"] + labels[:4]
                values = [data["load_seconds"]] + values[:4]

    fig, ax = plt.subplots(figsize=(5.5, 3.2))
    bars = ax.bar(range(len(labels)), values, color=ACCENT, edgecolor="white", linewidth=0.6)
    ax.set_xticks(range(len(labels)))
    ax.set_xticklabels(labels, rotation=25, ha="right", fontsize=8)
    ax.set_title("HF Sample Baseline (environment-specific)", fontsize=10, color=TEXT)
    ax.set_ylabel("value", fontsize=9)
    ax.spines[["top", "right"]].set_visible(False)
    ax.bar_label(bars, fmt="%.2g", fontsize=7, padding=2)
    fig.tight_layout()
    fig.savefig(OUT / "benchmarks.pdf", bbox_inches="tight")
    plt.close(fig)
...
def main() -> int:
    ...
    # Pipeline diagrams are native TikZ in sections/*.tex.
    # Only data-driven charts remain here.
    benchmarks()
