...
def daily_counts(root: Path, table: str, max_dates: int = 400) -> dict[str, int]:
...
def plot_daily_volume(poly_daily: dict[str, int], binance_daily: dict[str, int], out: Path) -> None:
    ...
    if not dates:
        return
    # Show the most recent observed days on a true date axis; gaps remain visible.
    dates = dates[-30:]
    x = [datetime.strptime(d, "%Y-%m-%d") for d in dates]
    fig, ax = plt.subplots(figsize=(7.2, 3.0))
    # Markers only: connecting lines across multi-week archive gaps would
    # visually imply continuous coverage that does not exist.
    ax.plot(x, [poly_daily.get(d, 0) for d in dates], marker="o", ms=4,
            ls="none", label="polymarket_ticks_ms", color="#f97316")
    ax.plot(x, [binance_daily.get(d, 0) for d in dates], marker="s", ms=4,
            ls="none", label="binance_trades", color="#4f46e5")
    ...
    ax.set_title("Daily Event Volume (30 most recent observed days)")
    ax.legend(fontsize=8, frameon=False)
...
def observed_day_stats(poly_daily: dict[str, int], binance_daily: dict[str, int]) -> dict:
    all_days = sorted(set(poly_daily) | set(binance_daily))
    if not all_days:
        return {}
    first = datetime.strptime(all_days[0], "%Y-%m-%d")
    last = datetime.strptime(all_days[-1], "%Y-%m-%d")
    return {
        "polymarket_observed_days": len(poly_daily),
        "binance_observed_days": len(binance_daily),
        "first_observed_day": all_days[0],
        "last_observed_day": all_days[-1],
        "observed_span_days": (last - first).days + 1,
    }
...
def write_tex(stats: dict, path: Path) -> None:
    ...
    lines = [
        ...
        f"\\newcommand{{\\OpenMarketPolyObservedDays}}{{{obs.get('polymarket_observed_days', 0)}}}",
        f"\\newcommand{{\\OpenMarketBinanceObservedDays}}{{{obs.get('binance_observed_days', 0)}}}",
        f"\\newcommand{{\\OpenMarketDataFirstDay}}{{{obs.get('first_observed_day', '')}}}",
        f"\\newcommand{{\\OpenMarketDataLastDay}}{{{obs.get('last_observed_day', '')}}}",
        f"\\newcommand{{\\OpenMarketDataSpanDays}}{{{obs.get('observed_span_days', 0)}}}",
        "",
    ]
...
def main() -> int:
    ...
    stats = {
        ...
        "observed_days": observed_day_stats(poly_daily, binance_daily),
