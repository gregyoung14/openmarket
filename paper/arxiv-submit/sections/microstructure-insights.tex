...
Hour-of-day medians (Figure~\ref{fig:lag-hour}) and daily event volumes
(Figure~\ref{fig:daily-volume}) show modest intra-day variation without a
simple ``high volume $\Rightarrow$ faster cross-venue adjustment'' rule.
...
On 354{,}684 step3 rows exported from unified Parquet, we compare forecast
baselines (Table~\ref{tab:forecast-benchmarks}, Figure~\ref{fig:forecast-benchmarks}).
...
\begin{table}[t]
  ...
  \begin{tabular}{@{}lrrrrr@{}}
    ...
    \midrule
    Naive \texttt{market\_mid\_prior\_up} & 0.842 & 0.162 & 0.015 & 0.484 & -- \\
    Logistic + Platt (\texttt{v0.2})      & 0.843 & 0.162 & \OpenMarketLogisticEce{} & 0.487 & \OpenMarketLogisticScoreUs{} \\
    \texttt{drift\_prob\_up} only         & 0.773 & 0.218 & 0.144 & 0.790 & -- \\
    \texttt{imbalance\_60s} (sigmoid)     & 0.586 & 0.246 & 0.031 & 0.685 & -- \\
    \bottomrule
  \end{tabular}
\end{table}

\textbf{Effect size.} Logistic \texttt{v0.2} beats the naive mid prior by
$\Delta$AUC $= \OpenMarketAucDiff{}$ (paired bootstrap 95\% CI
$[\OpenMarketAucDiffLow{}, \OpenMarketAucDiffHigh{}]$; one-sided
$p < 0.001$, $n=354{,}684$). The lift is statistically
