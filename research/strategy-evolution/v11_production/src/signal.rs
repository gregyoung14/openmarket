...
pub fn compute_signal(
    ...
    // ── Component 1: Drift Signal ──
    // Extrapolate observed log-return drift forward in time.
    // Uses μ * √(remaining_time) / σ as the z-score for a normal CDF.
    // The √time scaling comes from the random walk assumption:
    // the expected drift scales linearly with time,
    // but uncertainty scales with √time.
