...
impl CalibrationBin {
    pub fn win_rate(&self) -> f64 {
        if self.count == 0 { 0.0 } else { self.wins as f64 / self.count as f64 }
    }
...
impl CalibrationTable {
    ...
    pub fn new(bin_width: f64, min_sample: usize, entry_price: f64) -> Self {
        ...
        while lo < 1.0 - 1e-9 {
            ...
            bins.push(CalibrationBin { lo, hi, count: 0, wins: 0 });
            lo = hi;
        }
        Self { bins, min_sample, entry_price }
    }
    ...
    pub fn overall_brier(&self) -> f64 {
        ...
        if total == 0 { 0.0 } else { sum_sq / total as f64 }
    }
}
...
mod tests {
    ...
    fn test_brier_perfect_calibration() {
        ...
        for _ in 0..8 { breaker.record(0.80, 1.0); }
        for _ in 0..2 { breaker.record(0.80, 0.0); }
    ...
    fn test_brier_bad_calibration_trips() {
        ...
        for _ in 0..4 { breaker.record(0.90, 1.0); }
        for _ in 0..6 { breaker.record(0.90, 0.0); }
    ...
    fn test_calibration_table_bins() {
        ...
        for _ in 0..8 { table.record(0.72, true); }
        for _ in 0..2 { table.record(0.72, false); }
    ...
    fn test_dynamic_min_confidence() {
        ...
        for _ in 0..9 { table.record(0.57, true); }
        for _ in 0..11 { table.record(0.57, false); }
        // Bin 0.60-0.65: 20 trades, 13 wins (65% WR) → positive edge
        for _ in 0..13 { table.record(0.62, true); }
        for _ in 0..7 { table.record(0.62, false); }
