...
pub fn compute_signal(
    ...
    let mut combined_logit = *W_DRIFT * drift_logit
        + *W_OFI_ACCEL * ofi_logit
        + *W_SCOREBOARD * scoreboard_logit;
        ...
        if res.confidence >= min_confidence {
            ...
            if confirm_count >= res.adaptive_confirm {
                ...
                let bsvol = if res.regime != Regime::Neutral {
                    ...
                    let v = log_rets.iter().map(|&r| (r - m).powi(2)).sum::<f64>() / log_rets.len().max(1) as f64;
                    v.sqrt().max(0.00001)
                } else {
                    0.0001
                };
                ...
                let price_decayed = binary_call_price(current_price, btc_start, (t - 60.0).max(0.001), bsvol);
