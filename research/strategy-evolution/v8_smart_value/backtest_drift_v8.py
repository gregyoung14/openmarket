...
_PROJECT_ROOT = _os.path.join(_SCRIPT_DIR, '..', '..')
DB_PATH = _os.path.join(_PROJECT_ROOT, 'data', 'polymarket_btc_data.db')

# ========================================================================
# CONFIG - v8 Smart Value Settings
# ========================================================================
...
# Drift signal thresholds (tuned from v7 results)
MIN_DRIFT_CONFIDENCE = 0.65
...
CONFIRMATION_WINDOW  = 45   # Maintained v7's strong confirm

# Risk & Value (NEW)
MAX_OPEN_POSITIONS   = 1
MAX_DAILY_LOSS_PCT   = 0.20
MAX_ENTRY_PRICE      = 0.75 # Lowered from 0.99 to 0.75 to prevent huge R:R losses
MIN_EDGE             = 0.05 # Confidence must exceed (Price + Slippage) by 5%
...
def compute_drift_signal(trades_window, open_price, entry_seconds, remaining_seconds):
    ...
    dt = entry_seconds / len(log_returns)
    mu = np.mean(log_returns) / (dt + 1e-9)
    sigma = np.std(log_returns) / (np.sqrt(dt) + 1e-9)
    ...
    scoreboard_signal = 1 / (1 + np.exp(-price_vs_open * 5000))
    ...
    # --- Weighted Combination (v7 Weights) ---
    w_drift = 0.45
    w_scoreboard = 0.25
    w_ofi = 0.20
    w_ema = 0.10
...
# ========================================================================
# MARKET-LEVEL SIGNAL GENERATION
# ========================================================================
...
def build_market_signals(df_meta, df_trades, df_ticks):
    ...
    for i, market in df_meta.iterrows():
        ...
        # Get BTC ground truth for this window (OPTIMIZATION: slice once)
        # Note: We need trades BEFORE start_ms to compute drift correctly?
        # Actually drift is computed from start_ms to current_ms.
        # But `df_trades` in v7 loop used `trade_time >= start_ms`.
        # So it ignores history before market open. That's consistent.
        
        # We grab a slightly larger window just in case, or exact.
        # The loop query was: trade_time >= start_ms & trade_time < current_ms
        # So we can safely slice >= start_ms and < end_ms once.
        ...
        btc_start = market_trades.iloc[0]['price']
        btc_end = market_trades.iloc[-1]['price']
        ...
        for s in range(MIN_SECS_INTO_MARKET, MAX_SECS_INTO_MARKET):
            ...
            # OPTIMIZATION: Slice from the pre-filtered market_trades
            entry_trades = market_trades[market_trades['trade_time'] < current_ms]
...
def backtest_hold_to_resolve(signals, bankroll, bet_frac, slippage, fee_rate, min_conf,
    ...
    for s in signals:
        ...
        # 2. Price Cap Check (Hard Limit)
        if entry_ask > MAX_ENTRY_PRICE:
            # Skip expensive contracts regardless of confidence
            continue

        entry_price = entry_ask + slippage
        
        # 3. EV / Edge Check (Crucial v8 Logic)
        # We need (Confidence) > (Entry Price + Edge)
        # Example: Conf 0.70, Price 0.60 -> Edge 0.10 -> Trade
        # Example: Conf 0.60, Price 0.60 -> Edge 0.00 -> Skip
...
"""
    ...
    _html_path = _os.path.join(_SCRIPT_DIR, 'drift_backtest_results.html')
    with open(_html_path, 'w') as f:
        f.write(html)
    print(f"\n  Chart saved to {_html_path}")
...
def main():
    ...
    if best_log:
        ...
        _log_path = _os.path.join(_SCRIPT_DIR, 'drift_trade_log.csv')
        df_log.to_csv(_log_path, index=False)
        print(f"\n  Trade log saved to {_log_path} (best: {best_name})")
    
    if hold_sweep:
        _sweep_path = _os.path.join(_SCRIPT_DIR, 'drift_confidence_sweep.csv')
        pd.DataFrame(hold_sweep).to_csv(_sweep_path, index=False)
        print(f"  Confidence sweep saved to {_sweep_path}")
