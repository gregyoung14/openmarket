        ...
        "market_info" | "new_market" => {
            ...
            Ok(vec![PolymarketMessage::MarketInfo {
                ...
                question: value
                    ...
                    .and_then(|v| v.as_str())
                    .map(ToString::to_string),
                token_ids: if token_ids.is_empty() { None } else { Some(token_ids) },
                end_date: value
                    .get("end_date")
                    .and_then(|v| v.as_str())
                    .map(ToString::to_string),
...
mod tests {
    ...
    fn parses_real_wire_price_change_message() {
        ...
        match msg.into_iter().next().unwrap() {
            PolymarketMessage::PriceChange {
                ...
                assert_eq!(best_bid, Some(0.45), "best_bid should preserve explicit field");
    ...
    fn connected_message_parses_via_serde() {
        ...
        assert!(
            matches!(msg.into_iter().next().unwrap(), PolymarketMessage::Connected { .. }),
    ...
    fn official_price_change_array_expands_to_multiple_messages() {
        ...
        let messages = parse_polymarket_message(json).expect("official array price_change should parse");
        assert_eq!(messages.len(), 2);

        match &messages[0] {
            PolymarketMessage::PriceChange { asset_id, best_ask, .. } => {
    ...
    fn new_market_accepts_assets_ids_alias() {
        ...
        match msg.into_iter().next().unwrap() {
            PolymarketMessage::MarketInfo { slug, token_ids, .. } => {
                assert_eq!(slug.as_deref(), Some("btc-updown-15m-1771695900"));
