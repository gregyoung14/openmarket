...
async fn main() -> anyhow::Result<()> {
    ...
    let private_key = std::env::var(config::PRIVATE_KEY_ENV)
        .or_else(|_| {
            // Fallback: read from wallet-gen .env.wallet
            let wallet_path = "/path/to/wallet-gen/.env.wallet";
            std::fs::read_to_string(wallet_path).map(|contents| {
                contents
                    .lines()
                    .find(|l| l.starts_with("PRIVATE_KEY="))
                    .map(|l| l.trim_start_matches("PRIVATE_KEY=").to_string())
                    .unwrap_or_default()
            })
        })
        .expect("Set POLYMARKET_PRIVATE_KEY or ensure wallet-gen/.env.wallet exists");
...
mod tests {
    ...
    fn update_market_context_sets_tokens_and_end_time() {
        ...
        update_market_context(
            ...
            MarketInfoMsg {
                ...
                question: None,
                token_ids: Some(vec!["up_token_123".to_string(), "down_token_456".to_string()]),
                end_date: None,
            },
        );

        let ctx = state.market_context.lock().clone().expect("market context should exist");
        ...
        assert_eq!(ctx.market_end_ms, (1771695900_i64 + config::MARKET_DURATION_SECS) * 1000);
    }
    ...
    async fn signal_processing_loop_smoke_updates_context_and_prices() {
        ...
        let ctx = state.market_context.lock().clone().expect("market context should be set");
        let prices = state.live_prices.lock().clone();
        assert_eq!(ctx.up_token_id, "up_token_123");
