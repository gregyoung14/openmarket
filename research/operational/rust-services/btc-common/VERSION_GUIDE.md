...
> **Single source of truth:** `rust-services/btc-common/src/version.rs`
...
```
btc-common/src/version.rs   (source of truth)
        │
        ...
        └──► Python redeem-positions service
               ...
               Falls back to hardcoded "v14" if file is missing.
```
...
### 1. `rust-services/btc-common/src/version.rs`
...
### 2. `rust-services/btc-common/build.rs`
...
```bash
cd rust-services/signal-engine && cargo build --release
cd rust-services/execution-engine && cargo build --release
```
...
- `rust-services/execution-engine/src/models.rs` — `ENTRY_JSON` constant and assertions
- `rust-services/execution-engine/src/position.rs` — test helper `make_entry_signal` and JSON fixtures

Search for the old version string and replace:
```bash
grep -rn '"<old_version>"' rust-services/execution-engine/src/ --include="*.rs"
```

### 5. Restart services

```bash
systemctl --user restart signal-engine execution-engine redeem-positions
```
...
1. Edit **`rust-services/btc-common/src/version.rs`** — update `SIGNAL_VERSION` and `SIGNAL_METHOD`
2. Edit **`rust-services/btc-common/build.rs`** — update the matching string literals (`signal_version`, `signal_method`)
3. Search for the **old version string** in `rust-services/execution-engine/src/` and replace all occurrences in test fixtures
...
7. Restart all three services: `systemctl --user restart signal-engine execution-engine redeem-positions`
8. Confirm via `/health` endpoints

**Do NOT** add version strings anywhere else. If a new service needs the
version, add `btc-common = { path = "../btc-common" }` to its `Cargo.toml`
