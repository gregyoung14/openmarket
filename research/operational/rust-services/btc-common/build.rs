/// build.rs — writes version.json to the repo root so the Python
/// redeem-positions service can read the same constants.
///
...
fn main() {
  ...
  "signal_method": "{}"
    ...
    // Walk up from OUT_DIR to find the repo root (contains rust-services/)
    let out_dir = env::var("CARGO_MANIFEST_DIR").unwrap();
    let manifest_dir = PathBuf::from(&out_dir);

    // btc-common lives at rust-services/btc-common, so repo root is ../../
