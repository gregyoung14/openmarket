...
impl PaperState {
                    ...
                    if reader.read_line(&mut first_line).is_ok() {
                        ...
                        if !existing_header.is_empty() && existing_header != CSV_HEADER {
                            let rotated_path = self
                                .log_path
                                .with_extension(format!("legacy-{}.csv", Utc::now().timestamp_millis()));
    ...
    fn process_entry(&mut self, entry: &EntrySignal, market_end_ms: i64) -> Option<PendingTrade> {
        ...
        let ts = entry.timestamp.unwrap_or_else(|| Utc::now().timestamp_millis());
    ...
    fn metrics_summary(&self) -> String {
        ...
        let win_rate =
            (self.trades_won as f64 / self.trades_total as f64) * 100.0;
...
fn parse_gamma_market_outcome(market: &serde_json::Value) -> Option<SettledOutcome> {
    ...
    let up_idx: usize = if outcomes.len() >= 2 {
        outcomes
            .iter()
            .position(|o| {
                o == "yes" || o == "up" || o.contains("higher") || o.contains("above")
            })
            .unwrap_or(0)
    } else {
        0
...
async fn main() -> anyhow::Result<()> {
    ...
    info!("  Kelly Sizing: Quarter-Kelly ({:.2}x multiplier)", KELLY_MULTIPLIER);
    ...
    let token_side_map: Arc<Mutex<HashMap<String, String>>> =
