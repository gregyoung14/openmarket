...
pub fn export_step3_binary_calibration_csv(options: Step3ExportOptions) -> Result<Step3ExportSummary> {
    let export_dir = config::export_dir();
    fs::create_dir_all(&export_dir)?;
    let conn = db::get_db_conn()?;
    ...
    let manifest_path = format!("{}/step3_binary_calibration_{}.manifest.json", export_dir, ts);
    let mut writer = Writer::from_path(&csv_path)?;
    let feature_names: Vec<String> = DEFAULT_FEATURE_NAMES.iter().map(|name| (*name).to_string()).collect();
    ...
    for market in &markets {
        ...
        let label_up_final = if market_close_price > market_open_price { 1 } else { 0 };

        let poly_ticks = load_market_poly_ticks(&conn, &market.slug, market.start_ms, market.end_ms)?;
        ...
        for secs_in in (15_u64..signal_engine::config::MARKET_DURATION_SECS as u64).step_by(5) {
            ...
            if up_best_bid <= 0.0 || up_best_ask <= 0.0 || down_best_bid <= 0.0 || down_best_ask <= 0.0 {
                continue;
            }

            let Some(bars) = build_1s_bars_from_arrays(&raw_close, &raw_buy_vol, &raw_sell_vol, secs_in) else {
                continue;
            };
            let secs_left = signal_engine::config::MARKET_DURATION_SECS as u64 - secs_in;
            let Some(signal) = compute_drift_signal_v14(&bars, market_open_price, secs_left as f64) else {
                continue;
            };
            ...
            record.extend(feature_values.into_iter().map(|value| format!("{:.8}", value)));
...
fn list_step3_markets(conn: &rusqlite::Connection, start_ts_ms: i64, end_ts_ms: i64) -> Result<Vec<Step3Market>> {
    let mut stmt = conn.prepare(
        "SELECT market_slug
         FROM market_meta
    ...
    let mut markets: Vec<Step3Market> = rows
        ...
        .filter_map(|slug| {
            ...
            if start_ms >= start_ts_ms && start_ms <= end_ts_ms {
                Some(Step3Market { slug, start_ms, end_ms })
            } else {
                None
            }
        })
