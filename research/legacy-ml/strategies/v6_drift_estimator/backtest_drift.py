...
# ========================================================================
# CONFIG - Same execution params as backtest_alpha.py
# ========================================================================
...
# Drift signal thresholds (tuned from test_polymarket_ml.py results)
MIN_DRIFT_CONFIDENCE = 0.60   # Minimum P(UP) or P(DOWN) from drift estimator
MIN_SCOREBOARD_MOVE  = 0.0000 # Minimum |price_vs_open| (0 = no filter)

# Timing
MIN_SECS_INTO_MARKET = 60
MAX_SECS_INTO_MARKET = 600
MARKET_DURATION_SECS = 900
CONFIRMATION_WINDOW  = 30   # Signal must be stable for this many seconds
...
def compute_drift_signal(trades_window, open_price, entry_seconds, remaining_seconds):
    ...
    # Drift gets the most weight (it's the mathematically grounded one)
    # Scoreboard confirms (are we already moving in the right direction?)
    # OFI adds order flow context
    # EMA adds trend context
    w_drift = 0.40
    w_scoreboard = 0.25
    w_ofi = 0.20
    w_ema = 0.15
...
"""
    ...
    out_path = os.path.join(os.path.dirname(__file__), 'drift_backtest_results.html')
