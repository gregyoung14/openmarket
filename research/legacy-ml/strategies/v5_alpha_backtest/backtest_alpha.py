...
# ========================================================================
# CONFIG — Matches Alpha Rust Execution Engine (Premium Spec)
# ========================================================================
...
# Signal thresholds (Alpha Premium)
MIN_CONFIDENCE      = 0.60       # Premium Alpha threshold
MIN_CONSISTENCY     = 0.60       # Premium Alpha threshold
SIGNAL_WINDOW       = 30         # Rolling window for aggregation (s)
MIN_PREDICTIONS     = 5          # Need >= 5 predictions before acting

# Timing
MIN_SECS_INTO_MARKET = 30        # Wait 30s for signal buildup
MAX_SECS_INTO_MARKET = 600       # Enter up to 10 min in
...
def build_market_signals(df_meta, df_trades, df_ticks, df_lag):
    ...
    Mirrors Alpha Rust execution engine (SignalAggregator.rs):
      - Rolling window: SIGNAL_WINDOW (30s)
      - Entry: as soon as threshold hit between 30s and 600s
      - Thresholds: MIN_CONFIDENCE=0.60, MIN_CONSISTENCY=0.60
    """
    ...
    for i, market in df_meta.iterrows():
        ...
        # We need ML predictions for the entire possible entry window (0 to 600s)
        search_end_ms = start_ms + MAX_SECS_INTO_MARKET * 1000
        signal_start_dt = pd.to_datetime(start_ms, unit='ms')
        signal_end_dt = pd.to_datetime(search_end_ms, unit='ms')
        
        # Pre-calculate ML for the market window to simulate rolling
        market_ml = df_ml[(df_ml.index >= signal_start_dt) & (df_ml.index < signal_end_dt)].copy()
        if len(market_ml) < MIN_PREDICTIONS:
            continue
            
        X_market = market_ml[feats]
        market_probs = predict_ensemble(xgb_m, lgb_m, meta_clf, X_market)
        market_ml['raw_prob'] = market_probs
        
        # Simulate Alpha Rolling Aggregator
        hit_signal = None
        
        # Start search from MIN_SECS_INTO_MARKET
        for s in range(MIN_SECS_INTO_MARKET, MAX_SECS_INTO_MARKET):
            # Check if we have data for this second
            current_ms = start_ms + (s * 1000)
            current_dt = pd.to_datetime(current_ms, unit='ms')
            
            # Use rolling window of SIGNAL_WINDOW (30s)
            window_start_dt = pd.to_datetime(current_ms - SIGNAL_WINDOW * 1000, unit='ms')
            rolling_window = market_ml[(market_ml.index >= window_start_dt) & (market_ml.index <= current_dt)]
            
            if len(rolling_window) < 5:
                continue
            
            # Aggregation logic
            avg_prob = rolling_window['raw_prob'].mean()
            if avg_prob > 0.5:
                direction = 'UP'
                confidence = avg_prob
                consistency = (rolling_window['raw_prob'] > 0.5).mean()
            else:
                direction = 'DOWN'
                confidence = 1 - avg_prob
                consistency = (rolling_window['raw_prob'] <= 0.5).mean()
            
            # Check Premium Alpha thresholds
            if confidence >= MIN_CONFIDENCE and consistency >= MIN_CONSISTENCY:
                # HIT! Find entry price at this exact moment or shortly after
                entry_ticks = p_ticks[
                    (p_ticks['market_slug'] == slug) & 
                    (p_ticks['source_ts_ms'] >= current_ms) & 
                    (p_ticks['source_ts_ms'] < current_ms + 10000) # Look ahead 10s for price
                ]
                
                # Get ASK for our side
                side_ticks = entry_ticks[entry_ticks['side_label'] == direction]
                if len(side_ticks) == 0:
                    # Fallback to general market median if no tick in next 10s
                    entry_ask = 0.50
                else:
                    entry_ask = side_ticks.iloc[0]['best_ask']
                
                # We also need trajectories for momentum from entry onwards
                ...
                hit_signal = {
                    'slug': slug,
                    'start_ms': current_ms, # Entry timestamp
                    'end_ms': end_ms,
                    'btc_start': btc_start,
                    'btc_end': btc_end,
...
<meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Polymarket BTC Alpha Backtest (Premium Spec)</title>
<style>
...
<body>
<h1>Polymarket BTC Alpha Backtest</h1>
<p class="subtitle">15-Minute Binary Options | Rolling Signal Aggregation | Premium Alpha Spec</p>
<div>
...
"""
    ...
    out_path = os.path.join(os.path.dirname(__file__), 'backtest_results_alpha.html')
...
"""
    ...
    out_path = os.path.join(os.path.dirname(__file__), 'portfolio_results_alpha.html')
    with open(out_path, 'w', encoding='utf-8') as f:
        f.write(html)
