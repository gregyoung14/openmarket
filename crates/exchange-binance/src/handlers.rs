...
pub async fn health_check_handler(State(state): State<AppState>) -> impl IntoResponse {
    ...
    let db_snapshot = tokio::task::spawn_blocking(|| {
        ...
        let last_db_write_ms: Option<i64> = conn
            .query_row("SELECT MAX(received_at) FROM binance_trades", [], |r| {
                r.get(0)
            })
            .ok()
            .flatten();
        let database_size_bytes = std::fs::metadata(config::DATABASE_FILE)
            .map(|m| m.len())
            .ok();
    ...
    let db_write_fresh =
        db_write_age_ms.is_some_and(|age| age <= (config::HEALTH_DB_STALE_SECS as i64 * 1000));
