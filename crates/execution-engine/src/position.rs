...
mod tests {
    ...
    fn test_kelly_high_edge_capped_at_5pct() {
        ...
        let pos = pm.create_position(&signal, &market, 0.46, wallet).unwrap();
    ...
    fn test_kelly_medium_edge_proportional() {
        ...
        let pos = pm.create_position(&signal, &market, 0.52, wallet).unwrap();
    ...
    fn test_kelly_low_edge_floored_at_1pct() {
        ...
        let pos = pm.create_position(&signal, &market, 0.53, wallet).unwrap();
    ...
    fn test_kelly_zero_edge_floored_at_1pct() {
        ...
        let pos = pm.create_position(&signal, &market, 0.45, wallet).unwrap();
    ...
    fn test_kelly_shares_computed_from_bet_minus_fees() {
        ...
        let kelly_fraction: f64 =
            (0.5_f64 * 0.14 * 0.66).clamp(config::KELLY_MIN_FRACTION, config::KELLY_MAX_FRACTION);
        ...
        let entry_price =
            config::ceil_decimals(0.52 + config::SLIPPAGE, config::ORDER_PRICE_DECIMALS);
