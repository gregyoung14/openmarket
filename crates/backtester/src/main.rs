...
use calibration::{
    BrierCircuitBreaker, CalibrationTable, BRIER_TRIP_THRESHOLD, BRIER_WINDOW_SIZE,
    CALIBRATION_BIN_WIDTH, CALIBRATION_MIN_SAMPLE,
};
use config::*;
...
fn load_data(db_path: &PathBuf) -> Result<(DataFrame, DataFrame, DataFrame)> {
    ...
    let mut stmt = conn
        .prepare("SELECT market_slug, first_seen_ms FROM market_meta ORDER BY first_seen_ms ASC")?;
...
fn build_entry_ask_ladders(mkt_ticks: &DataFrame, start_ms: i64) -> (Vec<f64>, Vec<f64>) {
    ...
    for i in 0..mkt_ticks.height() {
        ...
        let sec =
            ((times.get(i).unwrap() - start_ms) / 1000).clamp(0, MARKET_DURATION_SECS - 1) as usize;
...
fn calculate_ols_alpha_beta_p(returns: &[(f64, f64)]) -> (f64, f64, f64) {
    ...
    let se_alpha =
        s_err * (1.0 / n + (mean_x * mean_x) / (sum_xx - n * mean_x * mean_x + 1e-9)).sqrt();
...
fn main() -> Result<()> {
    ...
    println!(
        "  Entry ask bounds: {:.3}–{:.3}",
        args.min_entry_price, args.max_entry_price
    );
    println!();
    ...
    let mut cal_table =
        CalibrationTable::new(CALIBRATION_BIN_WIDTH, CALIBRATION_MIN_SAMPLE, avg_entry);
    ...
    table.add_row(row!["Market Beta (Exposure)", format!("{:.4}", beta)]);
    let sig_text = if p_value < 0.05 {
    ...
    table.add_row(row![
        "Brier Score (final)",
        format!("{:.4}", brier.current_brier)
    ]);
    table.add_row(row![
        "Brier Window",
        format!("{}/{}", brier.window_len(), BRIER_WINDOW_SIZE)
    ]);
    ...
    table.add_row(row![
        ...
        format!(
            "{:.2}",
            cal_table.dynamic_min_confidence(args.min_confidence)
        )
    ]);
    ...
    for regime in &["Trend", "Neutral"] {
        let regime_trades: Vec<&TradeLog> = final_trades
            .iter()
